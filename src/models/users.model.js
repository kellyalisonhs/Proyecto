...
export async function createUser(user) {
  //const {username, correo_electronico, password, usertype,question, answer} = user;
  const {username, correo_electronico, password, usertype} =user;
  // Hash de la contraseña proporcionada antes de almacenarla en la base de datos
  const hashedPassword = createHash('md5').update(password).digest('hex');
  
  const strSql = 'INSERT INTO user (name_u, email_u, passwd_u, type_u) VALUES (?, ?, ?, ?, ?, ?)';
  // para manejar errores en la consulta
  try {
    const [result] = await conn.query(strSql, [username, correo_electronico, password, usertype]);
